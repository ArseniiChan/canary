#!/usr/bin/env node
/* eslint-disable */
// Build canary_report.docx from the structured content of canary_report.md.
// Uses docx-js (npm install -g docx; run with NODE_PATH=$(npm root -g)).
//
// Spec: US Letter, 1" margins, Times New Roman 12pt body, 14pt section
// headings, double-spaced body, page numbers in footer, embedded figures.
//
// Tables, figure captions, and section headings are encoded directly here
// rather than parsed from the markdown — this keeps the output stable and
// gives us exact control over typography for an academic submission.
...
const FIG = (name) => path.join(ROOT, "reports/figures", name);
const OUT = path.join(ROOT, "reports/canary_report.docx");

const TIMES = "Times New Roman";
const BODY = 24;     // 12pt = 24 half-points
const H1 = 32;       // 16pt
const H2 = 28;       // 14pt
const SMALL = 20;    // 10pt
...
const border = (color = "000000", size = 1) => ({ style: BorderStyle.SINGLE, size, color });
const cellBorders = {
...
function p(text, opts = {}) {
  const runs = Array.isArray(text)
    ? text
    : [new TextRun({ text, font: TIMES, size: opts.size || BODY, bold: opts.bold, italics: opts.italics })];
...
function image(file, widthIn) {
  ...
  const widthPx = (widthIn || 6) * 96; // 96 dpi nominal
  const heightPx = Math.round(widthPx * 0.625); // 8x5 aspect from the source figs
  return new Paragraph({
...
children.push(new Paragraph({
  ...
  children: [new TextRun({
    text: "Repository: github.com/ArseniiChan/canary  ·  Frozen analysis spec: git tag validation-spec-frozen",
...
[
  ...
  "“Clean” defined operationally. A peer is admitted to a cohort iff it satisfies three rules: the CIK is not on an editable AAER deny-list sourced from public knowledge of SEC enforcement releases; the same CIK has filed no Form 10-K/A within five years post-filing; the CIK or company name is not on an editable class-action deny-list sourced from public knowledge of major settlements. Both deny-lists are committed to the repository at data/processed/aaer_denylist.txt and data/processed/classaction_denylist.txt.",
...
children.push(buildTable(
  ["#", "Company", "CIK", "FY", "Accession", "Filed", "SIC", "Days pre-discovery"],
  [
    ["1", "Enron Corp.", "0001024401", "2000", "0001024401-01-500010", "2001-04-02", "6200", "197"],
    ["2", "WorldCom Inc.", "0000723527", "2001", "0001005477-02-001226", "2002-03-13", "4813", "104"],
    ["3", "Tyco International Ltd.", "0000833444", "2001", "0000912057-01-544874", "2001-12-28", "3585", "157"],
    ["4", "HealthSouth Corp.", "0000785161", "2001", "0001005150-02-000448", "2002-03-27", "8060", "357"],
    ["5", "Valeant Pharmaceuticals", "0000885590", "2014", "0000885590-15-000015", "2015-02-25", "2834", "236"],
    ["6", "Lehman Brothers Holdings", "0000806085", "2007", "0001104659-08-005476", "2008-01-29", "6211", "230"],
  ],
  [600, 1700, 1100, 500, 1700, 1100, 700, 1100]
));
...
children.push(h2("6.1 Per-fraud rank with random-baseline comparison"));
children.push(p(
...
children.push(h2("6.2 Mann-Whitney U on per-sentence error distributions"));
children.push(p(
...
children.push(h2("6.3 Bootstrap CIs and null permutation"));
children.push(p(
...
children.push(h2("6.4 Post-hoc TF-IDF + truncated-SVD trivial baseline"));
[
...
children.push(h2("6.5 Post-hoc entity-masking ablation"));
[
...
children.push(h2("6.6 Pre-committed decision rule for the post-hoc TF-IDF baseline"));
children.push(p(
...
[
  ...
  "This is the correct outcome of the contract, not a workaround for missing data. The honest reading is also the most uncomfortable one: the pre-registered design was infeasible for the most famous fraud in the dataset, and the design freeze did not catch the consequence. That is a finding about pre-registration, not about fraud detection. It also illustrates a real generalizable problem in held-out evaluation under temporal constraints: when the held-out positive examples include the chronologically earliest one in the eligible universe, time-controlled training has no out-of-sample data to consume. Future work that attempts the same design should either expand the training universe to clean filings outside the held-out cohorts (which the LOCO rule would still permit, time-controlled), or accept exclusion as a possible outcome and frame the held-out set with that in mind.",
].forEach((t) => children.push(p(t, { indent: { firstLine: 360 } })));

children.push(h2("7.2 Per-fraud rank under the pre-registered autoencoder (5 of 6 cohorts)"));
children.push(p(
  "Table 2 reports the pre-registered autoencoder results, single-pass against the frozen spec. Rank 1 = highest mean per-sentence reconstruction error within the cohort. Random-baseline expected rank for cohort size N is (N+1)/2 ≈ 6–7. MW p is one-sided “fraud > peers”; effect size is rank-biserial correlation in [-1, 1].",
  { indent: { firstLine: 360 } }
));
children.push(caption("Table 2. Pre-registered autoencoder results (single-pass against frozen spec)."));
children.push(buildTable(
  ["Fraud", "Cohort N", "Rank", "MW p", "Effect", "Bootstrap 95% CI", "Train sents"],
  [
    ["Lehman (LEH)", "13", "3", "1.3 × 10⁻⁹", "+0.12", "[1, 6]", "3,367"],
    ["HealthSouth (HRC)", "12", "7", "1.0", "−0.17", "[4, 10]", "2,283"],
    ["Valeant (VRX)", "13", "8", "1.0", "−0.19", "[5, 11]", "4,327"],
    ["Tyco (TYC)", "13", "9", "1.0", "−0.14", "[6, 12]", "861"],
    ["WorldCom (WCOM)", "13", "12", "1.0", "−0.15", "[10, 13]", "1,477"],
  ],
  [1700, 800, 700, 1100, 800, 1300, 1100]
));
children.push(image("per_fraud_rank.png"));
children.push(caption("Figure 1. Per-fraud rank vs random-baseline expectation. Rank 1 = highest reconstruction error within cohort. Lehman is the only cohort below the random expectation; the other four are at or above it."));
children.push(image("bootstrap_ci.png"));
children.push(caption("Figure 2. Per-fraud rank with bootstrap 95% CI (1,000 filing-level resamples). Confidence intervals are wide on every cohort, consistent with N = 6 sample-size limitations."));
[
...
children.push(h2("7.3 Aggregate hit@k vs random baseline"));
children.push(image("hit_at_k.png"));
children.push(caption("Figure 3. Aggregate hit@k vs random baseline across the five LOCO cohorts. Observed hit-rates fall at or below the random baseline at every k. Hit@3 and hit@5 are entirely Lehman."));
children.push(p(
  "Across the five LOCO cohorts: hit@1 = 0.00, hit@3 = 0.20, hit@5 = 0.20, against random-baseline 0.08 / 0.23 / 0.39. The observed hit-rates fall at or below random expectation across all three thresholds. Hit@3 = 0.20 corresponds entirely to Lehman; hit@5 = 0.20 corresponds entirely to Lehman as well, since no other cohort's fraud reached rank 5.",
  { indent: { firstLine: 360 } }
));

children.push(h2("7.4 Trivial baseline matches or exceeds the autoencoder on every cohort"));
children.push(caption("Table 3. Post-hoc TF-IDF + SVD32 baseline vs pre-registered autoencoder, same five cohorts."));
children.push(buildTable(
...
children.push(image("ae_vs_tfidf_rank.png"));
children.push(caption("Figure 4. Per-fraud rank: pre-registered autoencoder vs post-hoc TF-IDF + SVD32 baseline. The trivial baseline matches the autoencoder on Valeant and WorldCom and outperforms it on Lehman, HealthSouth, and Tyco."));
children.push(image("ae_vs_tfidf_mw.png"));
children.push(caption("Figure 5. Per-cohort Mann-Whitney p (one-sided fraud > peers, log scale): autoencoder vs TF-IDF + SVD32. The trivial baseline produces a stronger signal on Lehman by 13 orders of magnitude."));
[
...
children.push(h2("7.5 Lehman's positive result is robust to entity masking"));
children.push(image("entity_masking_posthoc.png"));
children.push(caption("Figure 6. Post-hoc entity-masking ablation: original vs masked autoencoder rank by cohort. Lehman's rank is invariant under masking 93 mentions of company-specific tokens, which weakens the literal-name-leakage explanation for its positive result."));
children.push(p(
  "The post-hoc entity-masking ablation tests one specific contamination concern: did MiniLM's web-pretraining seed Lehman's positive rank by memorizing literal mentions of “Lehman”, “Fuld”, “Repo 105”, or auditor names? After masking 93 such mentions in Lehman's MD&A (and replacing each with the literal token [ENTITY]), re-encoding, and rescoring against Lehman's existing autoencoder, Lehman's rank is invariant: 3 of 13 → 3 of 13. The other four cohorts show small or no movement. The literal-name-leakage hypothesis fails to explain Lehman's positive result. This does not address topical leakage — MiniLM may still have seen general post-2008 commentary about repo accounting, off-balance-sheet vehicles, or investment-bank insolvency that influenced its representations. That residual contamination concern remains disclosed but unaddressed empirically.",
  { indent: { firstLine: 360 } }
));
children.push(p(
...
children.push(caption("Table 4. Post-hoc entity-masking on the five LOCO cohorts. “Δ” is masked rank − original rank."));
children.push(buildTable(
...
children.push(h2("7.6 Leave-one-fraud-out aggregate sensitivity"));
children.push(p(
...
children.push(h2("7.7 Honest interpretation"));
[
  "The numbers admit two readings, both of which are consistent with the data. First, the single signal has no real out-of-sample power at N = 6. What looks like a positive on Lehman is one observation drawn from the cohort with the largest training set (3,367 training sentences vs. Tyco's 861, a factor-of-four asymmetry), and bootstrap CIs are wide on every cohort. The TF-IDF baseline matching the autoencoder is consistent with this reading. Second, MD&A language is too heterogeneous for a single autoencoder reconstruction-error signal to discriminate fraud from clean peers without supervision. Sector- and year-matched peer cohorts are coarse — within a SIC-2-digit code, business models and disclosure styles vary widely.",
...
children.push(p("Three substantive findings emerged from this project.", { indent: { firstLine: 360 } }));
[
...
[
  ...
  "U.S. Securities and Exchange Commission. Accounting and Auditing Enforcement Releases (AAERs). https://www.sec.gov/divisions/enforce/friactions.shtml.",
    ...
    footers: {
      default: new Footer({
        children: [new Paragraph({
          alignment: AlignmentType.CENTER,
          children: [
            new TextRun({ text: "Canary — Arsenii Chan — CSC 44800 — ", font: TIMES, size: SMALL }),
            new TextRun({ text: "Page ", font: TIMES, size: SMALL }),
